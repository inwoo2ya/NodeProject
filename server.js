const timeTable = () => {
  ...
  const today = `${todate.getFullYear()}-${("0" + todate.getMonth() + 1).slice(
    -2
  )}-${todate.getDate()}`;
  return today;
};
...
let db;
MongoClient.connect(
  "mongodb+srv://admin:n37w0rk@project0.ebrquly.mongodb.net/?retryWrites=true&w=majority",
  (err, client) => {
    if (err) {
      console.log(err);
    } else {
...
app.delete("/delete", (req, res) => {
  ...
  db.collection("data1").deleteOne(req.body, (err, result) => {
    if (err) {
      console.log(err);
    } else {
      console.log("삭제완료");
